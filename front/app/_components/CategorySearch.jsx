...
import { getDoctorCategoryImage } from "@/(route)/doctorCategory/card/doctorCategory/doctorCategoryUrl-image";

function CategorySearch({ DoctorCategory }) {
  const [categoryList, setCategoryList] = useState();
  useEffect(() => {
    getCategoryList;
  }, []);
  const doctorCategory = DoctorCategory
      ...
      <div className=" grid grid-cols-2 mt-5ّ md:grid-cols-3 lg:grid-cols-4">
        {doctorCategory.length > 0 ? doctorCategory.map((data, i) => {
          return (
            ...
            >
              <img
                src={getDoctorCategoryImage(data.id)}
                //  src={data.image}
