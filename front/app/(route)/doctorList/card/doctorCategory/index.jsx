...
import { deleteDoctorList } from "../../api/doctorList.api";
import { Button } from "../../../../../components/ui/button";
import { getDoctorListImage } from "./product-image";

function DoctorListIndex({ DoctorList }) {
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
  const [selectedId, setSelectedId] = useState();

  const handleDelete = async () => {
    await deleteDoctorList(selectedId);
            ...
            {DoctorList.map((item, key) => (
              <tr key={item.id}>
                ...
                <td className="flex justify-center items-center gap-x-3 mt-4">
                  <img
                    src={getDoctorListImage(item.id)}
                ...
                <td>{item.name}</td>
                <td>{item.category}</td>
                <td>{item.yearsOfExprience}</td>
                <td>{item.address}</td>
                <td className="flex justify-center items-center gap-x-3">
                  <Link
                    href={`/doctorList/mod/edit/${item.id}`}
                    className="w-fit"
                  >
                    <EditIcon />
                  </Link>
...
export default DoctorListIndex;